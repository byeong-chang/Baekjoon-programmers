...
### 3회 시도
최적화 그리디 문제이다.
올바른 풀이를 위한 조건을 찾아야한다.

최적화를 위해 이미 멀티탭 안에 있는 값 중에서 다시는 불리지 않을 놈을 삭제하거나,
최대한 늦게 불리는 놈을 삭제하는 것을 구현한다면 가능할듯.

구현부에서 남은 처리값 중에서 index를 통해 몇번째에 있는지 찾을 수 있다는 생각이 들어 예외처리로 간단히 구현했음.  
다만,가장 멀리 있는 값을 저장하지 않는 실수를 해서 3회시도함.
