...
### 4회 시도 
첫 풀이는 입력받아서 R 값이면 카운트를 증가시키고  
카운트가 홀수일때만 reverse를 쓰도록   
D를 만나면 pop(0)을 하되  
예외처리로 error 출력을 하도록 구현했다.  
- reverse를 너무 자주사용하였는지 시간초과로 틀린 풀이.  
- 알고리즘 분류를 보니 Deque 자료구조를 사용하는 것으로 확인이 되었음.  

deque를 사용하여 다시 구현한 후 제출했는데 문제가 없어 보임에도 틀렸다고 하더군.  
출력부를 int형 list로 출력하면 정답이 아니고 문자열로 출력해야 정답이 되더라 억까문제였다.  

### 성능 요약

메모리: 42692 KB, 시간: 160 ms
