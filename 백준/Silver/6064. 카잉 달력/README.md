...
### 5회이상 시도  
- 문제 이해가 개 어려운 문제였다.
- 처음에는 최소 공배수를 구해서(이게 잉카 달력의 맥시멈 값이라서) 이 값보다 작거나 같은 경우에 대해서만 체크하는 방식을 적용했다.  
- 체크 시에는 x % M 과 y % N 을 나눈 나머지의 값을 공통으로 가지는 K 를 구하는 문제였다.  
- 중국인의 나머지 정리 공식을 공부하고 적용하려 했으나 역원에 대한 구현이 어려워 bruteforce로 풀이를 시도해 맞았다.
