...
### 6회시도
처음 접근했던 방식은 모든 경우에 대해 조사해서 얻어낸 값들을 list로 만들어 max값을 제출하려 했으나 시간 초과가 발생했습니다.
그 후 알고리즘 분류를 확인해보니 DP임을 알 수 있었고, DP 풀이법으로 접근하여 풀이함
DP 특성상 index error가 발생하였는데, 입력값이 작은 경우에 발생하는 에러를 잡지 못해서 런타임 에러가 발생했음.
