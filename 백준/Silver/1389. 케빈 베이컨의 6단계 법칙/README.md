...
### 2회 시도 BFS 또는 floyd warshall알고리즘 
floyd warshall 알고리즘을 2번정도 접했었는데, 이 문제에 적용하기 까지 오랜 기간이 걸렸다. 관련 문제를 한번정도 더 풀어봐야 이해가 될 것 같다. 
### 문제 고찰
1. 각 index에서 케빈 베이컨 조사 후 list 형태로 저장
2. index(min(조사된 list))를 출력하면 종료

### 조사를 어떻게 하냐?
- brute force는 너무 많지 않을까
- 100 X 100 행렬을 2개 만들어서 모든 경우 조사하기엔 2초는 시간초과날듯
- 메모리는 여유가 있어보여서 DP를 쓸 수는 있을듯
- -> DP로 풀어볼까?
- -> 알고리즘 확인 후 
- Graph 이론의 Floyd warshall 알고리즘으로 풀거나 BFS로 풀라고 되어있음.
- BFS는 어제 해봤으니까 Floyd warshall 알고리즘을 써보자!
